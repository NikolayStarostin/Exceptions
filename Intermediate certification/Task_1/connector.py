        ...
        try:
            if program == tag_out:
                ...
                try:
                    ...
                    if choice == 'id':
                        search = int(input('Введите id заметки:'))
                        search_note(us_list, search, 1)
                    elif choice == 'title':
                        search = str(input('Введите "Заголовок" заметки:'))
                        ssearch_note(us_list, search, 2)
                    elif choice == 'msg':
                        search = str(input('Введите "Текст заметки" для поиска (Например: Сегодня солнечно):'))
                        search_note(us_list, search, 3)
                    elif choice == 'data':
                        search = str(input('Введите "Полную дату" для поиска (Например: 06-02-2023 18:23:59):'))
                        search_note(us_list, search, 4)
